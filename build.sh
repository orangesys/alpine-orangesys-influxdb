...
for path in $dockerfiles; do
  ...
  log_msg "${tag}/influxdb-${tag}-static_linux_amd64.tar.gz"
  if ! docker_build -t "orangesys/alpine-orangesys-influxdb:${tag}" "$path"; then
    failed_builds+=("$tag")
  fi
done

if [ ${#failed_builds[@]} -eq 0 ]; then
  docker tag orangesys/alpine-orangesys-influxdb:${tag} orangesys/alpine-orangesys-influxdb:latest
  find . -name influxdb*.tar.gz| xargs rm -f
